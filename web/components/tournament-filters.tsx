...
function FilterBody({
  ...
  filters: Filters;
  update: (partial: Partial<Filters>) => void;
  reset: () => void;
  hasHomeClub?: boolean;
  hasFavoriteClubs?: boolean;
}) {
  ...
  return (
    <div className="mb-4">
      ...
      <div className="sm:hidden">
        ...
        {sheetOpen && (
          <div
            className="fixed inset-0 z-[10000] bg-black/40 flex items-end"
          ...
          >
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 pb-[calc(env(safe-area-inset-bottom)+5rem)]">
                <FilterBody
                  filters={filters}
                  update={update}
                  reset={reset}
                  hasHomeClub={hasHomeClub}
                  hasFavoriteClubs={hasFavoriteClubs}
                />
              </div>
              <div className="border-t border-gray-100 px-4 py-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] bg-white">
                <button
                  onClick={() => setSheetOpen(false)}
                  className="w-full py-2.5 text-sm font-semibold bg-accent text-white rounded-lg"
                >
                  Ergebnisse anzeigen
                </button>
              </div>
      ...
      {/* Desktop: keep the existing collapsible details panel */}
      <details className="group bg-white border border-gray-200 rounded-lg hidden sm:block">
